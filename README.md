...
В данном форке внедрен **Автоматический патч Silent Audio Engine**, который полностью решает эту проблему [travel]! При сборке с флагом **`-c la`** скрипт автоматически вшивает в Swift-код генератор бесконечной тишины [travel]. Для iOS приложение выглядит как активный музыкальный плеер, что предотвращает заморозку процесса и позволяет Rust-ядру работать бесконечно [travel]!
...
2. **Конфликт при звонках:** При совершении обычного сотового звонка или вызова в мессенджерах iOS принудительно глушит наш фейковый аудио-поток [travel]. **После окончания разговора необходимо перезапустить прокси вручную (нажать Стоп -> Старт)**, чтобы вернуть многозадачность.

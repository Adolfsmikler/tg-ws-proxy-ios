...
3.  **Рекомендуемый экспериментальный режим:** На бесплатном аккаунте сборка с флагом **`-c la`** (Live Activity) и патчем плеера (`audio`) работает дольше, но со временем трафик всё равно **затухает и обрывается** из-за ограничений песочницы iOS на локальные сокеты (127.0.0.1). На текущий момент полностью стабильная работа «из коробки» без VPN-профиля не достигнута. 
